...
#' get_retweets_of_user(users, "2020-01-01T00:00:00Z", "2020-04-05T00:00:00Z", bearer_token, data_path = "data/")
#' }
get_retweets_of_user <- function(users, start_tweets, end_tweets, bearer_token, file = NULL, data_path = NULL){
  ...
  while (!is.null(nextoken)) {
    query <- paste0('retweets_of:', users[[i]])
    userhandle <- users[[i]]
    df <-
      get_tweets(
        q = query ,
