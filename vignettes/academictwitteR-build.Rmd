...
But what if we were only interested in a particular region?
...
The `point_radius` argument requires three pieces of information: the longitude and latitude of a target coordinate, and the buffer size around that coordinate.
...
As such, if we wanted to search for tweets containing all the words "dog" and "cat" and "balls" then we would do the following:

```{r, eval = F}

tweets <-
  get_all_tweets(
    query = c("dog cats balls"),
    start_tweets = "2019-12-31T10:00:00Z",
    end_tweets = "2020-01-01T10:00:00Z"
  )
...
However, we could also search user tweets and filter by what they're saying at the same time. Here, we would be getting tweets by users @cbarrie and @jack when they were particularly excited, for example:
