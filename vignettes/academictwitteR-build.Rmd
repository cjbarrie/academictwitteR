...
Note that `build_query()` can also take a character vector of strings as query. This will produce the equivalent of typing `"OR"` between our target strings. Let's say we were interested in multiple hashtags and not just one. We could still build a query filter our tweets by the parameters discuss above as follows:
