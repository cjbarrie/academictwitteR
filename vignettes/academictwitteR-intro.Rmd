...
This vignette provides a quick tour of the R package `academictwitteR`. The package is useful solely for querying the Twitter Academic Research Product Track v2. API endpoint. 
...
This query will only capture a maximum of 200 tweets as we have not changed the default 

If you have not set your bearer token, you can do so within the function as follows:
...
tweets <-
  get_all_tweets(
    ...
    start_tweets = "2020-01-01T00:00:00Z",
    end_tweets = "2020-01-05T00:00:00Z",
    bearer_token = "AAAAAAAAAAAAAAAAAAAAAPwXWFFlLLDVC6G0PFo4shkDVg02DwVxGQIVKvhPVE3vdV",
    file = "blmtweets"
  )
...
Note finally that here we are setting an upper limit of tweets of one million. The default limit is set to 200. For almost all applications, users will wish to change this. 
